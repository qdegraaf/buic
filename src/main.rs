use std::fs::File;
use std::io::Write;
use buic::args::{Buic, BuicCommand, WeatherCommand};
use buic::precipitation::get_rain;
use clap::Parser;


#[tokio::main]
async fn main() -> Result<(), reqwest::Error>{
    let args = Buic::parse();

    match args.cmd {
        BuicCommand::Rain {latitude, longitude } => {
            let response = get_rain(latitude, longitude).await;
            match args.output {
                Some(path) => {
                    // TODO: Add better error handling
                    // TODO: Add multiple file type handling
                    let mut file = File::create(path).expect("Could not create file for path given path");
                    file.write_all(response.as_ref()).expect("Could not write response to file");
                },
                None => println!("{}", response)
            }
        }
        BuicCommand::Weather { cmd } => match cmd {
            WeatherCommand::Actuals { station } => println!("{:?}", station),
            WeatherCommand::Forecast { n_days } => println!("{:?}", n_days)
        }
    }
    println!("Done");

    Ok(())
}
